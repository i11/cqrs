...
import static com.bobkevic.cqrs.publisher.dtos.Message.MAP_TYPE_REFERENCE;
...
import com.bobkevic.cqrs.publisher.ApiToCompletableFutureHelper;
  ...
  public CompletableFuture<String> store(final String destination, final Message message) {
    return supplyAsync(() ->
        PubsubMessage.newBuilder()
            .setData(getMessageData(message))
            .putAllAttributes(getMessageAttributes(message))
            .build())
        .thenCompose(pubsubMessage -> publisherCache.get(destination)
            .thenCompose(publisher ->
                ApiToCompletableFutureHelper.toCompletableFuture(publisher.publish(pubsubMessage))
            ));
  }

  Map<String, String> getMessageAttributes(final Message message) {
    final ImmutableMap.Builder<String, String> attributesBuilder =
        ImmutableMap.<String, String>builder()
            .put("correlation_id", message.correlationId().orElseGet(UUID::randomUUID).toString());

    message.attributes().ifPresent(attributesBuilder::putAll);
    return attributesBuilder.build();
  }

  ByteString getMessageData(final Message message) {
    final ImmutableMap.Builder<String, Object> repositoryMessageDataBuilder =
        ImmutableMap.<String, Object>builder().put("@type", message.name());
    return message.messageAsMap(json)
        .map(msg -> repositoryMessageDataBuilder.putAll(msg).build())
        .map(map -> Json.uncheckedSerialization(json, map))
        .map(ByteString::copyFromUtf8)
        .orElseGet(() ->
            ByteString
                .copyFromUtf8(uncheckedSerialization(json, repositoryMessageDataBuilder.build())));
  }
